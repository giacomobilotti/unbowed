...
arrows <- read_excel(file.path(sourcedir, "Arrows_Ethnographic_v_Archaeological_updated.xlsx"))

# mix group and type
arrows <- arrows |>
  rename(Width = Width, Thickness = Thickness) |>  
  mutate(
    Group = case_when(
...
eth  <- arrows |> 
  filter(Group == "Ethnographic arrows")
arch <- arrows |>
  filter(Group == "Archaeological arrows")
...
### Data viz ----
# Define Color/shape palette
pal <- c(
  "Archaeological darts"   = "#0077BB",  
  "Ethnographic arrows"    = "#EE7733",  
  "Archaeological arrows"  = "#009988"   
)

shape_vals <- c(
  "Archaeological darts"   = 2,    
  "Ethnographic arrows"    = 1,    
  "Archaeological arrows"  = 17   
)
...
## Figure S6  ----
# width vs. thickness scatterplot
vda_label <- sprintf(
...
ggsave(file.path("figures", "Figure_SI6.svg"), p_hull,
       width = 7.5, height = 6.5, device = "svg")
...
ggsave(file.path("figures", "Figure_SI8.svg"), p_dens,
       width = 7, height = 5, device = "svg")

## summarise stat results ----
# # uncomment this to print the stat summary 
...
# # overlap. Darts stand clearly apart.
# cat("\nThickness (mm), distribution-free summary:\n")
# print(stats_t, n = Inf)
# 
# # --- Wilcoxon rank-sum test ---
# # The test checks if the two grops are different.
# # Both p-values are very small — the difference is not due to chance.
# # Note that sample size differences might have an impact here
# cat("\nWilcoxon rank-sum test (ethnographic vs. archaeological arrows):\n")
...
# # How different are our samples. Bootstrapping within group
...
# 
# # All three test independently suggest a difference between the ethnographic and arch data.